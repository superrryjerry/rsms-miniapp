...
Page({
  ...
  async loadData() {
    ...
    try {
      const res = await app.request({ url: '/vehicles/list', data: { page: this.data.page, size: 20, keyword: this.data.keyword } });
      if (res.code === 0) {
  ...
  goDetail(e) {
    wx.navigateTo({ url: '/pages/vehicle/detail?vin=' + e.currentTarget.dataset.vin });
  }
});
